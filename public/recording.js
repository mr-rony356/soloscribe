...
// Form Selected Event Listeners

document.addEventListener('DOMContentLoaded', function() {
    const formSelectors = {
      'upload-file': 'upload-section',
      'from-podcast-library': 'podcast-link-form',
      'from-youtube-link': 'youtube-link-form',
      'record-audio': 'record-form'
    };
  
    function showForm(formId) {
      // Hide all forms
      Object.values(formSelectors).forEach(id => {
        const formElement = document.getElementById(id);
        if (!formElement) {
          console.error(`Form element with ID "${id}" not found`);
          return;
        }
      ...
      if (!selectedForm) {
        console.error(`Selected form with ID "${formId}" not found`);
        return;
      }
      selectedForm.style.display = 'block';
    }
  
    // Initialize event listeners for each form selector
    Object.keys(formSelectors).forEach(selectorId => {
      const selectorElement = document.getElementById(selectorId);
      if (!selectorElement) {
        console.error(`Selector element with ID "${selectorId}" not found`);
        return;
      }
      selectorElement.addEventListener('click', function() {
        showForm(formSelectors[selectorId]);
      });
    });
  });
